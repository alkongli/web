  ...
  <div>
    ...
    <div>
      ...
      <div style="margin-top: 10px;background-color: red" class="code">
        ...
        <div class="order" style="color: white">超值低价，秒杀抢购</div>
        <span class="order" style="color: white;">
          <span style="font-size: 30px">￥{{productInfo.price*productInfo.discount}}</span>
          <span style="font-size: 30px;color: #eaeaea;text-decoration: line-through">￥{{productInfo.price}}</span>
        </span>
        <div style="height: 5px"></div>
      ...
      <div class="footer">
        ...
        <div class="title" @click="orderAdd()">立即秒杀</div>
      </div>
    </div>
  </div>
</template>
  ...
  methods:{
    ...
    orderAdd(){
      if(this.color===''||this.size===''){
        ...
        orders.amount=this.num*this.productInfo.price*this.productInfo.discount
        orders.productId=this.productInfo.id
        orders.orderProduct=[]
        ...
        orderProduct.price=this.productInfo.price*this.productInfo.discount
        ...
        this.$axios.post("/api/secKill",orders).then(res2=>{
          if (res2.data.code === 1) {
