<template>
  <el-container>
    <el-aside width="100px" >
      <el-menu :default-active="$store.state.productList">
        <el-menu-item v-for="item in productType" :key="item.id" :index="item.id" @click="changeType(item.id)">{{item.type}}</el-menu-item>
      </el-menu>
    </el-aside>
    <el-main>
...
export default {
  ...
  created() {
    ...
    if(this.$store.state.productList==='')
      ...
      this.changeType(this.$store.state.productList)
  },
  methods:{
    changeType(typeId){
      this.$axios.get("/api/product/typeId",{
        ...
        this.$store.dispatch('productList',typeId)
      })
